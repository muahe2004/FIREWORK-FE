...
import { IBaseDelete, IBaseResponse, ICheckCodeResponse } from "@/types/base";
...
export const checkProductCode = async (product_code: string): Promise<ICheckCodeResponse> => {
  const res = await apiClient?.get(`${prefix}/check-code`, {params: {product_code}});
  return res.data;
};