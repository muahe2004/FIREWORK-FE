...
import { ERROR_TIMEOUT, LOCAL_USER, SEARCH_CONTENT } from "@/constant/config";
...
import { getFunctionsByUser } from "@/services/user.service";
...
export default function RoleTable({
  ...
  const [func, setFunction] = useRecoilState(functionRoleState);
  ...
  // Handle set new features
  const handleSetNewFeatures = async () => {
    const newFeatures = await getFunctionsByUser(userRecoil.user_id);

    const user = _.clone(userRecoil);
    user.functions = newFeatures;

    setUserRecoil(user);
    storageService.setStorage(LOCAL_USER, JSON.stringify(user));
  };
